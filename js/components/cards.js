...
(function () {
  ...
  function createChannelCard(channel) {
    ...
    return '<div class="card channel-card" data-stream-id="' + escapeAttr(String(channel.stream_id || '')) + '" data-type="channel" data-epg-channel-id="' + escapeAttr(String(channel.epg_channel_id || '')) + '" data-focusable tabindex="0" role="button" aria-label="' + escapeAttr(name) + '" style="flex-shrink:0;width:140px;">' +
      ...
      '<img data-src="' + escapeAttr(icon) + '" alt="' + escapeAttr(name) + '" style="width:100%;height:100%;object-fit:contain;padding:4px;" loading="lazy" onerror="this.style.display=\'none\';this.parentNode.innerHTML=\'<span style=color:#555;font-size:24px;font-weight:700>\' + escapeHtml(name.charAt(0).toUpperCase()) + '</span>\'">' +
      '</div>' +
