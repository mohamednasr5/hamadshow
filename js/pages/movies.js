 ...
 * row of poster cards with a "See All" link. Selecting "See All" (or
 * searching) switches to a full grid for that scope, with a back button
 * to return to the category rows.
 */
(function () {
  'use strict';
  ...
  function movieCardHTML(m) {
    ...
    var badges = '';
    if (year) badges += '<span class="content-card-badge">' + esc(year) + '</span>';
    if (rating) {
    ...
    return '<div class="modal-overlay" id="movie-detail-overlay">' +
      '<div class="modal-panel" style="max-width:560px;max-height:90vh;overflow-y:auto">' +
        ...
        '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;font-size:0.8125rem;color:var(--text-secondary)">' +
          ...
          (duration ? '<span>' + (window.i18n ? window.i18n.t('movies.duration') : 'Duration') + ': ' + esc(duration) + '</span>' : '') +
        '</div>' +
        (plot ? '<p style="font-size:0.875rem;color:var(--text-secondary);line-height:1.7;margin-bottom:16px">' + esc(plot) + '</p>' : '') +
        (director ? '<p style="font-size:0.8125rem;color:var(--text-tertiary);margin-bottom:4px"><strong>' + (window.i18n ? window.i18n.t('movies.director') : 'Director') + ':</strong> ' + esc(director) + '</p>' : '') +
  ...
  function render(container) {
    ...
    var allLabel = window.i18n ? window.i18n.t('movies.all') : 'All';
    ...
    container.innerHTML =
      '<div class="movies-page" style="padding:8px 16px 24px">' +
        ...
        '</div>' +
        '<div id="movies-back-bar" style="display:none;align-items:center;gap:10px;margin-bottom:14px">' +
          '<button class="btn btn-ghost btn-sm" id="movies-back-btn">' +
          ...
          '</button>' +
          '<h2 id="movies-cat-title" style="font-size:1.1rem;font-weight:700;color:var(--text-primary)"></h2>' +
        '</div>' +
        '<div id="movies-rows"><div id="movies-rows-skeletons">' + skeletonRow() + skeletonRow() + skeletonRow() + '</div></div>' +
        '<div id="movies-grid-wrap" style="display:none">' +
          '<div id="movies-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px">' +
            skeletonGrid(18) +
          '</div>' +
          ...
          '<div id="movies-loadmore" style="display:none;text-align:center;padding:20px">' +
            '<button class="btn btn-secondary" id="movies-loadmore-btn">' + seeAllLabel + '</button>' +
          '</div>' +
        '</div>' +
      '</div>';

    var rowsEl = container.querySelector('#movies-rows');
    var gridWrapEl = container.querySelector('#movies-grid-wrap');
    ...
    var backBar = container.querySelector('#movies-back-bar');
    var backBtn = container.querySelector('#movies-back-btn');
    var catTitleEl = container.querySelector('#movies-cat-title');
    ...
    function categoryRowHTML(cat, items) {
      ...
      var cards = shown.map(movieCardHTML).join('');
    ...
    function renderRows() {
      ...
      categories.forEach(function (cat) {
        var items = allMovies.filter(function (m) { return String(m.category_id) === String(cat.category_id); });
        if (items.length === 0) return;
        html += categoryRowHTML(cat, items);
      });
      if (!html) {
        html = '<div class="empty-state"><div class="empty-state-title">' + (window.i18n ? window.i18n.t('movies.noMovies') : 'No movies found') + '</div></div>';
      }
    ...
    api.getVodStreams().then(function (streams) {
      allMovies = streams || [];
      var rowsSkeletons = container.querySelector('#movies-rows-skeletons');
