...
(function() {
  ...
  class PlayerManager {
    ...
    _detectUrlType(url) {
      ...
      if (/\.(flv)($|\?)/i.test(url) || cleanUrl.endsWith('.flv')) return 'flv';
      return 'other';
    }
    ...
    _loadStream(url, strategy) {
      ...
      // ─── MPEG-TS / FLV via mpegts.js ───
      if (strategy === 'mpegts') {
        this._playWithMpegTs(url);
        return;
      }
    ...
    _pickBestStrategy(urlType) {
      ...
      if (urlType === 'flv') {
        if (typeof window.mpegts !== 'undefined' && window.mpegts.isSupported()) return 'mpegts';
        return 'native-src';
      }
    ...
    /**
     * Strategy: Play MPEG-TS or FLV streams using mpegts.js library.
     * This is the most reliable way to play .ts live streams in browsers.
     */
    _playWithMpegTs(url) {
      if (typeof window.mpegts === 'undefined' || !window.mpegts.isSupported()) {
        console.warn('[Player] mpegts.js not available, skipping');
        this._tryNextFallback();
        return;
      }

      var self = this;
      console.log('[Player] Trying mpegts.js:', url);
      ...
      this.mpegts.on(window.mpegts.Events.ERROR, function(errorType, detail) {
        console.warn('[Player] mpegts.js error:', errorType, detail);
        if (errorType === window.mpegts.ErrorTypes.NETWORK_ERROR ||
            errorType === window.mpegts.ErrorTypes.MEDIA_ERROR) {
          self._tryNextFallback();
        }
      });

      this.video.addEventListener('loadeddata', function() {
        console.log('[Player] mpegts.js data loaded');
        self._attemptPlay();
      }, { once: true });

      // Timeout after 12 seconds
      setTimeout(function() {
        if (self.video.readyState < 2 && !self.hls && !self.dash) {
          console.warn('[Player] mpegts.js timeout');
          self._tryNextFallback();
        }
      }, 12000);
    }
    ...
    _onVideoError() {
      ...
      if (this.hls || this.dash || this.mpegts) return;
    ...
    _tryNextFallback() {
      ...
      var strategies = {
        ...
        'ts': ['mpegts', 'native-src', 'native-source', 'hlsjs-manifest'],
        'flv': ['mpegts'],
      ...
      for (var i = 0; i < allUrls.length; i++) {
        ...
        for (var j = 0; j < urlStrategies.length; j++) {
          ...
          // Skip mpegts if not available
          if (strategy === 'mpegts' && !(typeof window.mpegts !== 'undefined' && window.mpegts.isSupported())) continue;
